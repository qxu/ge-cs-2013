...
public class DungeonMaze extends BasicGame {
	...
	public void init(GameContainer container) throws SlickException {
		...
		if (visionTracker == Tournament.vision.length) { // END GAME
			...
			for (int v = 0; v < Tournament.vision.length; v++) {
				System.out.println("--- Results for Vision Distance Set at "
						+ Tournament.vision[v] + " ---");
				System.out.print("\t");
			...
			System.out.println("Vision distance: "
					+ Tournament.vision[visionTracker]);
		}
	}
		...
		if (playerMoveTime <= 0 && gameRunning) {
			...
			if (player.end() || (steps >= Tournament.maxSteps)) {
				if (steps >= Tournament.maxSteps) {
					...
					System.out.println("Escaped the dungeon in " + steps
							+ " steps :)");
					// update scoreboard
	...
	public void render(GameContainer container, Graphics g) {
		map.getMap()
				.render(320 - (int) ((float) player.getPlayerGridLocation()
						.getX() * 16),
						240 - (int) ((float) player.getPlayerGridLocation()
								.getY() * 16));
