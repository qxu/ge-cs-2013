...
public class PlayerVision
{
	...
	public PlayerVision(Map map, Point playerLoc)
	{
		...
		{
			mWest++;
			curPoint = map.getMapBox(new Point((int)playerLoc.getX() - mWest,
					(int)playerLoc.getY()));
		}
		...
		{
			mEast++;
			curPoint = map.getMapBox(new Point((int)playerLoc.getX() + mEast,
					(int)playerLoc.getY()));
		}
